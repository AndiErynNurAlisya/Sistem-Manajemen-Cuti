{{-- resources/views/leader/leave-requests/show.blade.php --}}
{{-- SAMA dengan employee show, hanya approval timeline berbeda (hanya HRD, tanpa leader) --}}
<x-app-layout>
    ...
    <div x-data="{ showCancelModal: false, cancellationReason: '' }">
        ...
        {{-- CONTENT SAMA DENGAN EMPLOYEE SHOW --}}
        {{-- Copy grid structure dari employee show --}}
        {{-- Hanya bagian approval timeline yang berbeda (hanya HRD) --}}
        
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {{-- Main Content: COPY FROM EMPLOYEE SHOW --}}
            {{-- Leave Information Card --}}
            {{-- Leave Details Card --}}
            {{-- Approval Timeline Card (modified untuk leader) --}}
            <div class="lg:col-span-2 space-y-6">
                {{-- ... same cards as employee ... --}}
                    ...
                    <div class="p-6">
                        @if($leaveRequest->approvals->isEmpty())
                            <div class="text-center py-8">
                                ...
                                <p class="text-sm text-gray-500">Menunggu approval HRD</p>
                            </div>
                        @else
                            <div class="flow-root">
                                <ul class="-mb-8">
                                                    ...
                                                    <div class="flex-1 min-w-0">
                                                        <div>
                                                            <div class="text-sm">
                                                                ...
                                                                <span class="text-gray-500 ml-2">(HRD)</span>
                                                            </div>
                                                        ...
                                                        @if($approval->status === 'approved')
                                                            <div class="mt-2 text-sm text-green-700 bg-green-50 rounded-md p-3">
                                                                <p class="font-medium">✓ Disetujui HRD</p>
                                                                @if($approval->notes)
                                                        ...
                                                        @else
                                                            <div class="mt-2 text-sm text-red-700 bg-red-50 rounded-md p-3">
                                                                <p class="font-medium">✗ Ditolak HRD</p>
                                                                @if($approval->notes)
            ...
            {{-- Sidebar Actions: SAME AS EMPLOYEE --}}
            <div class="lg:col-span-1 space-y-6">
                <div class="bg-white rounded-lg shadow-sm p-6">
                    <h3 class="text-lg font-semibold text-gray-900 mb-4">Aksi</h3>
                    <div class="space-y-3">
                        {{-- Download buttons same as employee --}}
                        {{-- Cancel button same as employee --}}
                        ...
                        <div class="ml-3">
                            <p class="text-sm text-blue-700">
                                @if($leaveRequest->status->value === 'pending')
                                    Pengajuan sedang menunggu approval dari HRD.
                                @elseif($leaveRequest->status->value === 'approved')
                                    Pengajuan cuti Anda telah disetujui HRD! Silakan download surat izin.
                                @elseif($leaveRequest->status->value === 'rejected')
                                    Pengajuan cuti ditolak oleh HRD. Anda bisa mengajukan cuti baru.
                                @else
